...
public class LoginManager {
    ...
    private void openActivity(Context context, String loginUserEmail){
        Intent intent = new Intent(context,ShoppingActivity.class);
        intent.putExtra(String.valueOf(R.id.txtEmail), loginUserEmail);
    ...
    public void signIn(Context context, User user) throws LoginException{
        if(loginUsingSP(context, user)) {
            openActivity(context, loginUser.getEmail());
            return;
        }
            ...
            openActivity(context, user.getEmail());
        }
        else
